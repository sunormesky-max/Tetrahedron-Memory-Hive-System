...
class AgentMemoryLoop:
    ...
    def get_evolution_report(self) -> Dict[str, Any]:
        ...
        with field.field_lock():
            stats = field.stats()
            fb_stats = field.feedback_loop_get_stats()
            so_stats = field.self_organize_stats()
            dp_stats = field.dark_plane_stats()
            vc_stats = field.void_channel_stats()
            reg_status = field.self_regulation_status()
            obs_stats = field.observer_stats()
    ...
    def get_proactive_suggestions(
        ...
        with field.field_lock():
            ...
            dp_state = field.dark_plane_substrate_state()
            if dp_state is not None:
                pe = dp_state["persistent_entropy"]
                coherence = dp_state["coherence"]
                last_pt = dp_state["last_phase_transition"]
                ...
                if last_pt and time.time() - last_pt < 300:
                    suggestions.append({
                        ...
                        "transition_level": dp_state["total_phase_transitions"],
                    })
                ...
                vc_ref = field.void_channel_ref()
                if vc_ref is not None:
                    vc_stats = vc_ref.get_stats()
            ...
            reg = field.self_regulation_status()
            if reg:
            ...
            insights = field.feedback_loop_get_insights()
            for ins in insights:
            ...
            if clusters > 0:
                weak_clusters = []
                for cl in field.self_organize_clusters():
            ...
            top_edges = field.hebbian_top_edges(5)
            if top_edges:
                high_success_pairs = [
                    {
                        "source": e["source"][:12],
    ...
    def run_targeted_cycle(self, target_domain: str) -> Dict[str, Any]:
        ...
        with field.field_lock():
            ...
            domain_nodes = []
            domain_labels = set()
            for nid, node in field.all_nodes_items():
            ...
            if dp_state is not None:
                ...
                for nid, _ in domain_nodes[:20]:
                    channels = field.void_channel_for_node(nid)
                    if channels:
                        domain_in_dark = True
                        break
                dark_plane_context = {
        ...
        with field.field_lock():
            ...
            if vc_ref is not None:
                for nid, _ in nodes[:30]:
                    void_channel_count += len(field.void_channel_for_node(nid))
    ...
    def auto_optimize(self) -> Dict[str, Any]:
        ...
        if should_regulate:
            try:
                reg_result = field.self_regulation_regulate()
                reg_status_after = field.self_regulation_status()
                result["regulation_trigger"] = {
                    "stress_before": reg_status_after.get("stress", {}).get("level", 0),
    ...
    def _snapshot_metrics(self) -> Dict[str, Any]:
        field = self._field
        stats = field.stats()
        fb = field.feedback_loop_get_stats()
        dp_state = field.dark_plane_substrate_state()
        dp = {}
        if dp_state is not None:
            dp = {
                "pe": dp_state["persistent_entropy"],
                "coherence": dp_state["coherence"],
                "void_energy": dp_state["void_energy"],
                "h4_growth": dp_state["h4"]["growth_rate"],
            }
        return {
    ...
    def _sense_dark_plane(self, field: "HoneycombNeuralField") -> Dict[str, Any]:
        ...
        result["pe"] = dp_state["persistent_entropy"]
        result["coherence"] = dp_state["coherence"]
        result["void_energy"] = dp_state["void_energy"]
        result["dark_energy"] = dp_state["dark_energy"]
        result["channel_energy"] = dp_state["channel_energy"]
        result["h3"] = dp_state["h3"]
        result["h4"] = dp_state["h4"]
        result["h5"] = dp_state["h5"]
        result["h6"] = dp_state["h6"]
        result["phase_transitions"] = dp_state["total_phase_transitions"]
        result["last_transition"] = dp_state["last_phase_transition"]
        result["cascade_potential"] = dp_state["cascade_potential"]
        result["psi_field"] = field.dark_plane_psi_field()
        result["cross_dim_coupling"] = field.dark_plane_cross_dim_coupling()
    ...
    def _sense_runtime_observer(self, field: "HoneycombNeuralField") -> Dict[str, Any]:
        ...
        obs_ref = field.observer_ref()
        if obs_ref is None:
            return result
        obs = obs_ref.get_stats()
        result["available"] = True
    ...
    def _phase_sense(self, field: "HoneycombNeuralField") -> Dict[str, Any]:
        ...
        all_occupied = field.occupied_items()
        ...
        for action in plan.get("planned_actions", []):
            try:
                if action["type"] == "create_bridge":
                    ...
                    for nid, n in field.all_nodes_items():
                    ...
                    vc_ref = field.void_channel_ref()
                    if vc_ref is not None:
                        active = vc_ref.get_active_channels(min_dimension=2)
                        if active:
                            strongest = max(active, key=lambda ch: ch.strength)
                            node_a = field.node_get(strongest.node_a)
                            node_b = field.node_get(strongest.node_b)
    ...
    def _phase_learn(
        ...
        fb_after = field.feedback_loop_get_stats()
