    ...
    if ui_password and hmac.compare_digest(ui_password, _ui_password):
        if _ui_password == "CHANGE_ME":
            ...
            if client_host not in ("127.0.0.1", "::1", "localhost"):
                ...
                raise HTTPException(401, "Default password only allowed from localhost. Set TETRAMEM_UI_PASSWORD env var.")
        token = state.auth_manager.create_ui_token()
        state.metrics.increment("auth_successes")
        return {"token": token, "mode": "ui", "password_is_default": _ui_password == "CHANGE_ME"}
...
def restore_backup(backup_id: str, request: Request):
    ...
    try:
        with state.state_lock:
            state.field.import_full_state(restored)
    except Exception as e:
        log.error("Failed to import backup state: %s", e)
        raise HTTPException(500, "Backup loaded but state import failed")
...
def system_status(request: Request):
    ...
    return {
        "system_health": state.system_ops.last_health_check or {"status": "no_check_yet"},
