...
def store(req: StoreReq, request: Request):
    ...
    try:
        ...
        s.log_op("store", {"id": tid, "content": req.content, "labels": req.labels or [], "weight": req.weight, "centroid": s.field.node_get(tid).position.tolist(), "metadata": req.metadata or {}, "creation_time": float(s.field.node_get(tid).creation_time)})
        if s.persistence is not None:
            s.persistence.log_operation("store", {"id": tid, "content": req.content, "labels": req.labels, "weight": req.weight, "metadata": req.metadata, "centroid": s.field.node_get(tid).position.tolist(), "creation_time": float(s.field.node_get(tid).creation_time)})
...
def batch_store(request: Request, req: List[Dict[str, Any]]):
    ...
    for item in req:
        try:
            content = InputValidator.sanitize_content(item.get("content", ""))
            labels = item.get("labels", [])
            weight = item.get("weight", 1.0)
            metadata = item.get("metadata", {})
            valid, err = InputValidator.validate_store(content, labels, weight, metadata)
            if not valid:
                results.append({"content": content[:50], "status": "error", "error": err})
                continue
            with s.state_lock:
...
def delete_tetra(tetra_id: str, request: Request):
    ...
    tenant = getattr(request.state, "tenant", {"tenant_id": "default"})
    with s.state_lock:
        node = s.field.node_get(tetra_id)
        if node is None:
            raise HTTPException(404, "Not found")
        if not node.is_occupied:
            raise HTTPException(404, "Not found")
        s.field.clear_node(tetra_id)
    ...
    if seed:
        ...
        with s.state_lock:
            ...
            while frontier and steps < max_steps:
                next_frontier = []
                for nid in frontier:
                    n = s.field.node_get(nid)
                    if n is None:
                        continue
                    for fn in n.face_neighbors:
                        if fn not in visited and s.field.node_exists(fn):
                            visited.append(fn)
                            next_frontier.append(fn)
                    for en in n.edge_neighbors[:4]:
                        if en not in visited and s.field.node_exists(en):
                            visited.append(en)
