...
class BackupManager:
    ...
    def _safe_path(self, backup_id: str) -> Optional[str]:
        if ".." in backup_id or "/" in backup_id or "\\" in backup_id:
            return None
        if not backup_id.startswith("backup_") or not backup_id.endswith(".json"):
            return None
        path = os.path.join(self._storage_dir, backup_id)
        real = os.path.realpath(path)
        if not real.startswith(os.path.realpath(self._storage_dir)):
            return None
        return path

    def restore_backup(self, backup_id: str) -> Optional[dict]:
        path = self._safe_path(backup_id)
