...
class HoneycombNeuralField:
    ...
    def __init__(self, resolution: int = 5, spacing: float = 1.0):
        ...
        self._pending_store_effects: List[Tuple[str, float, float]] = []
    ...
    def store(self, content: str, labels: Optional[List[str]] = None,
        ...
        with self._lock:
            ...
            if chash in self._content_hash_index:
                ...
                if existing and existing.is_occupied:
                    ...
                    if labels:
                        ...
                        existing._label_set = set(existing.labels)
                    if metadata:
            ...
            if self._pulse_engine is None or not self._pulse_engine.is_alive():
                self._emit_pulse(nid, strength=weight * 0.5, pulse_type=PulseType.REINFORCING)
                self._connect_temporal(nid, weight)
            else:
                self._pending_store_effects.append((nid, weight, time.time()))
    ...
    def restore_at_position(self, content: str, centroid: list,
        ...
        with self._lock:
            ...
            for lbl in node.labels:
                self._label_index[lbl].add(nid)
            r_tokens = self._extract_tokens(content)
            node._cached_tokens = r_tokens
            node._cached_trigrams = self._extract_ngrams(content, 3)
            node._label_set = set(node.labels)
            for tok in r_tokens:
                self._content_token_index[tok].add(nid)
            return nid
    ...
    def query(self, text: str, k: int = 5, labels=None) -> List[Dict]:
        ...
        with self._lock:
            cache_key = f"{text}|{k}|{sorted(labels) if labels else ''}"
            ...
            q_labels_set = set(labels) if labels else None
            for nid in candidate_ids:
                node = self._nodes.get(nid)
                if not node or not node.is_occupied:
                    continue
                if labels:
                    lbl_set = node._label_set if node._label_set is not None else set(node.labels)
                    if not q_labels_set & lbl_set:
                        continue

                text_score = 0.0
                if qtokens:
                    ctokens = node._cached_tokens if node._cached_tokens is not None else self._extract_tokens(node.content)
                    if ctokens:
                ...
                trigram_score = 0.0
                if qtrigrams:
                    ctrigrams = node._cached_trigrams if node._cached_trigrams is not None else self._extract_ngrams(node.content, 3)
                    if ctrigrams:
                ...
                if labels:
                    lbl_set = node._label_set if node._label_set is not None else set(node.labels)
                    overlap = len(q_labels_set & lbl_set)
                    label_score = overlap / max(len(q_labels_set | lbl_set), 1)
            ...
            top_k = _heapq.nlargest(k, scored, key=lambda x: x[1])
    ...
    def _flush_pending_store_effects(self):
        if not self._pending_store_effects:
            return
        with self._lock:
            batch = self._pending_store_effects[:]
            self._pending_store_effects.clear()
        for nid, weight, ts in batch:
            try:
                self._emit_pulse(nid, strength=weight * 0.5, pulse_type=PulseType.REINFORCING)
                self._connect_temporal(nid, weight)
            except Exception:
                logger.debug("deferred store effect failed for %s", nid[:8], exc_info=True)
    ...
    def stop_pulse_engine(self):
        ...
        self._flush_pending_store_effects()
    ...
    def _pulse_loop(self):
        ...
        while not self._stop_event.wait(timeout=self._adaptive_interval):
            try:
                ...
                logger.debug("flush pending store effects failed", exc_info=True)
        ...
        with self._lock:
            ...
            for nid, node in hot_empty[:5]:
                ...
                if len(sources) >= cfg.MIN_BRIDGE_SOURCES and not node.is_occupied:
                    ...
                    for lbl in node.labels:
                        self._label_index[lbl].add(nid)
                    bridge_tokens = self._extract_tokens(bridge)
                    node._cached_tokens = bridge_tokens
                    node._cached_trigrams = self._extract_ngrams(bridge, 3)
                    node._label_set = set(node.labels)
    ...
    def import_full_state(self, state_dict: Dict[str, Any]):
        with self._lock:
            ...
            for nid, node in list(self._nodes.items()):
                if node.is_occupied and nid not in nodes_data:
            ...
            for nid, ndata in nodes_data.items():
                ...
                for lbl in node.labels:
                    self._label_index[lbl].add(nid)
                if node.content:
                    restore_tokens = self._extract_tokens(node.content)
                    node._cached_tokens = restore_tokens
                    node._cached_trigrams = self._extract_ngrams(node.content, 3)
                    node._label_set = set(node.labels)
            ...
            if skipped > 0:
                logger.warning(
                    "import_full_state: %d/%d nodes skipped (not in current lattice)",
                    skipped, len(nodes_data),
                )
    ...
    def node_get(self, nid: str):
        return self._nodes.get(nid)

    def occupied_items(self):
        return [(nid, n) for nid, n in self._nodes.items() if n.is_occupied]

    def all_nodes_items(self):
        return list(self._nodes.items())

    def hebbian_top_edges(self, n: int = 5) -> List[Dict[str, Any]]:
        if self._hebbian is None:
            return []
        if not hasattr(self._hebbian, "_edges"):
            return []
        sorted_edges = sorted(
            self._hebbian._edges.items(),
            key=lambda x: -(x[1].get("weight", 0) if isinstance(x[1], dict) else x[1]),
        )
        results = []
        for (n1, n2), w in sorted_edges[:n]:
            nn1 = self._nodes.get(n1)
            nn2 = self._nodes.get(n2)
            if nn1 and nn2 and nn1.is_occupied and nn2.is_occupied:
                results.append({
                    "source": n1,
                    "target": n2,
                    "weight": float(w.get("weight", w) if isinstance(w, dict) else w),
                    "source_node": nn1,
                    "target_node": nn2,
                })
        return results

    def self_organize_clusters(self):
        if self._self_organize is None:
            return []
        if not hasattr(self._self_organize, "_clusters"):
            return []
        return list(self._self_organize._clusters)

    def dark_plane_substrate_state(self) -> Optional[Dict[str, Any]]:
        if self._dark_plane_substrate is None:
            return None
        s = self._dark_plane_substrate._state
        return {
            "persistent_entropy": s.persistent_entropy,
            "coherence": s.coherence,
            "void_energy": s.void_energy,
            "dark_energy": s.dark_energy,
            "channel_energy": s.channel_energy,
            "cascade_potential": s.cascade_potential,
            "h3": {"count": s.h3.count, "energy": s.h3.energy, "growth_rate": s.h3.growth_rate},
            "h4": {"count": s.h4.count, "energy": s.h4.energy, "growth_rate": s.h4.growth_rate},
            "h5": {"count": s.h5.count, "energy": s.h5.energy, "growth_rate": s.h5.growth_rate},
            "h6": {"count": s.h6.count, "energy": s.h6.energy, "growth_rate": s.h6.growth_rate},
            "total_phase_transitions": s.total_phase_transitions,
            "last_phase_transition": s.last_phase_transition,
        }

    def dark_plane_cross_dim_coupling(self) -> Dict[str, Any]:
        if self._dark_plane_substrate is None:
            return {}
        return self._dark_plane_substrate._compute_cross_dim_coupling()

    def dark_plane_psi_field(self) -> float:
        if self._dark_plane_substrate is None:
            return 0.0
        return self._dark_plane_substrate._psi_field

    def void_channel_ref(self):
        return self._void_channel

    def void_channel_for_node(self, nid: str) -> List:
        if self._void_channel is None:
            return []
        return self._void_channel.get_channels_for_node(nid)

    def feedback_loop_ref(self):
        return self._feedback_loop

    def feedback_loop_get_stats(self) -> Dict[str, Any]:
        if self._feedback_loop is None:
            return {}
        return self._feedback_loop.get_stats()

    def feedback_loop_get_insights(self) -> List[Dict[str, Any]]:
        if self._feedback_loop is None:
            return []
        return self._feedback_loop.get_learning_insights()

    def self_regulation_status(self) -> Dict[str, Any]:
        if self._self_regulation is None:
            return {}
        return self._self_regulation.status()

    def self_regulation_regulate(self):
        if self._self_regulation is None:
            return None
        return self._self_regulation.regulate()

    def node_exists(self, nid: str) -> bool:
        return nid in self._nodes

    def clear_node(self, tetra_id: str) -> bool:
        node = self._nodes.get(tetra_id)
        if node is None:
            return False
        self._clear_node(tetra_id, node)
        return True

    def all_nodes_values(self):
    ...
    def compute_node_geometric_quality(self, nid: str) -> float:
        return self._compute_node_geometric_quality(nid)

    def compute_geometric_topo_divergence(self, nid: str) -> float:
        return self._compute_geometric_topo_divergence(nid)

    def reflection_field_node_energy(self, nid: str) -> float:
        if self._reflection_field is None:
            return 0.5
        return self._reflection_field.get_node_energy(nid)

    def bcc_direction_factor(self, pos_a, pos_b) -> float:
        return self._bcc_direction_factor(pos_a, pos_b)

    def autocorrelation_history_len(self) -> int:
        return len(self._autocorrelation_history)

    def bcc_cell_coherence(self, nid: str) -> float:
        return self._bcc_cell_coherence(nid)

    def get_bcc_cellmates(self, nid: str) -> list:
        return self._get_bcc_cellmates(nid)

    def emergence_history_recent(self, n: int) -> list:
        return self._emergence_history[-n:]

    def check_convergence_bridges(self):
        self._check_convergence_bridges()

    def global_decay(self):
        self._global_decay()

    def is_node_occupied(self, nid: str) -> bool:
        n = self._nodes.get(nid)
        return n is not None and n.is_occupied
