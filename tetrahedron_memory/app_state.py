...
def _resolve_node(field, node_id: str) -> str:
    if field.node_exists(node_id):
        return node_id
    matches = [nid for nid, _ in field.all_nodes_items() if nid.startswith(node_id)]
    if len(matches) == 1:
        return matches[0]
    elif len(matches) > 1:
...
class AppState:
    ...
    def initialize(self) -> None:
        ...
        default_key = os.environ.get("TETRAMEM_DEFAULT_API_KEY", "")
        if default_key:
            self.auth_manager.register_raw_key(default_key, "default", "admin")
    ...
    def _proactive_loop(self) -> None:
        ...
        while not self.proactive_engine_stop.is_set():
            ...
            try:
                if cycle % 5 == 0:
                    s = self.field.self_check_status()
