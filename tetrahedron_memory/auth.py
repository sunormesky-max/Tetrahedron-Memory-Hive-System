...
class AuthManager:
    ...
    def resolve_key(self, key_hash: str) -> Optional[Dict]:
        return self._api_keys.get(key_hash)

    def register_raw_key(self, raw_key: str, tenant_id: str, role: str = "agent",
                         quota: int = 100000) -> None:
        key_hash = self.hash_key(raw_key)
        self._api_keys[key_hash] = {
            "tenant_id": tenant_id,
            "name": "pre-registered",
...
class TenantAwareField:
    ...
    def delete(self, tetra_id: str, tenant_id: Optional[str] = None) -> bool:
        effective_tenant = tenant_id or self._active_tenant
        if self._mode == TenantMode.SINGLE or effective_tenant is None:
            node = self._field.node_get(tetra_id)
            if node is None:
                return False
            self._field.clear_node(tetra_id)
            return True
        node = self._field.node_get(tetra_id)
        if node is None:
            return False
    ...
    def export_all_tenants(self) -> Dict[str, Dict]:
        ...
        for node in self._field.all_nodes_values():
