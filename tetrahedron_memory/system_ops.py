...
class SystemOperationManager:
    ...
    def verify_data_integrity(self) -> Dict[str, Any]:
        ...
        with self._field._lock:
            nodes_in_memory = {
                nid
                for nid, n in self._field._nodes.items()
                if n.is_occupied
            }
            ...
            invalid_edges = 0
            if hasattr(self._field, "_edges"):
                for n1, n2, etype in self._field._edges:
                    if n1 not in self._field._nodes or n2 not in self._field._nodes:
                        invalid_edges += 1
            if invalid_edges > 0:
                results["issues"].append(
            ...
            centroid_issues = 0
            spacing = getattr(self._field, "_spacing", 1.0)
            resolution = getattr(self._field, "_resolution", 5)
    ...
    def create_rollback_point(self, label: str) -> Dict[str, Any]:
        ...
        rollback_path = os.path.join(rollback_dir, f"{label}.json")
        tmp_fd, tmp_path = tempfile.mkstemp(
    ...
    def rollback_to(self, label: str) -> Dict[str, Any]:
        ...
        with self._field._lock:
            before_count = sum(
                1 for n in self._field._nodes.values() if n.is_occupied
            )
            self._field.import_full_state(state)
            after_count = sum(
                1 for n in self._field._nodes.values() if n.is_occupied
            ...
            with self._field._lock:
                for nid, node in self._field._nodes.items():
                    ...
                    for fnid in node.face_neighbors:
                        fn = self._field._nodes.get(fnid)
