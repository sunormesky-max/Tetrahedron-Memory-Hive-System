...
TOOLS = [
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "weight": {
                    "type": "number",
                    "description": "Importance: 1.0=normal, 1.5=important, 2.0=critical. Default 1.0.",
                    "default": 1.0
                }
            },
            "required": ["content"]
    ...
    class MCPSSEHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            if not self._check_auth():
                ...
                self.end_headers()
                self.wfile.write(json.dumps({"error": "Authentication required. Provide Authorization: Bearer <token> or X-API-Key header."}).encode())
                return
        ...
        def do_OPTIONS(self):
            ...
            self.send_header("Access-Control-Allow-Origin", "http://localhost:3000 http://localhost:8082")
